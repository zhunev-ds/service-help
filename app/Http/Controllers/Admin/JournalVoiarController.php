...
class JournalVoiarController extends Controller
{
    use MediaUploadingTrait;

    public function index(Request $request)
    {
        abort_if(Gate::denies('journal_voiar_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        if ($request->ajax()) {
            $query = Task::orderBy('due_date', 'desc')->with(['assigned_to', 'tags', 'status'])->select(sprintf('%s.*', (new Task)->table))->where('type', 'void');
        ...
        return view('admin.journalVoiars.index');
    }
}
