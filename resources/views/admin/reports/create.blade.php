    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reports.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quarter') ? 'is-invalid' : '' }}" type="text" name="quarter" id="quarter" value="{{ old('quarter', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('brigades') ? 'is-invalid' : '' }}" name="brigades[]" id="brigades" multiple>
                    @foreach($brigades as $id => $brigade)
                        <option value="{{ $id }}" {{ in_array($id, old('brigades', [])) ? 'selected' : '' }}>{{ $brigade }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('recommendations') ? 'is-invalid' : '' }}" name="recommendations" id="recommendations">{{ old('recommendations') }}</textarea>
