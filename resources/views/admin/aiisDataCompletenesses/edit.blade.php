...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.aiisDataCompleteness.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aiis-data-completenesses.update", [$aiisDataCompleteness->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="date">{{ trans('cruds.aiisDataCompleteness.fields.date') }}</label>
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $aiisDataCompleteness->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" type="number" name="state" id="state" value="{{ old('state', $aiisDataCompleteness->state) }}" step="1">
                @if($errors->has('state'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $aiisDataCompleteness->description) }}</textarea>
