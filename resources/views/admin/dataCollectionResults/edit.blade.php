...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.dataCollectionResult.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.data-collection-results.update", [$dataCollectionResult->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="date">{{ trans('cruds.dataCollectionResult.fields.date') }}</label>
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $dataCollectionResult->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('change_character') ? 'is-invalid' : '' }}" name="change_character" id="change_character">{{ old('change_character', $dataCollectionResult->change_character) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\DataCollectionResult::CONSIDERED_METROLOGICAL_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('considered_metrological') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="considered_metrological_{{ $key }}" name="considered_metrological" value="{{ $key }}" {{ old('considered_metrological', $dataCollectionResult->considered_metrological) === (string) $key ? 'checked' : '' }}>
