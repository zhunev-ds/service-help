...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.analysisWorkAii.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.analysis-work-aiis.update", [$analysisWorkAii->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('quarter') ? 'is-invalid' : '' }}" name="quarter_id" id="quarter_id">
                    @foreach($quarters as $id => $quarter)
                        <option value="{{ $id }}" {{ ($analysisWorkAii->quarter ? $analysisWorkAii->quarter->id : old('quarter_id')) == $id ? 'selected' : '' }}>{{ $quarter }}</option>
                    @endforeach
                </select>
