...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.visualInspectionOfAii.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.visual-inspection-of-aiis.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('point') ? 'is-invalid' : '' }}" name="point_id" id="point_id">
                    @foreach($points as $id => $point)
                        <option value="{{ $id }}" {{ old('point_id') == $id ? 'selected' : '' }}>{{ $point }}</option>
                    @endforeach
                </select>
                @if($errors->has('point'))
